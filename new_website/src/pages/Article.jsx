...
const Article = () => {
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      try {
        let res = await fetch(`/api/news/adult/${id}`);

        if (!res.ok) {
          res = await fetch(`/api/news/kids/${id}`);
        }
  ...
  const fetchSummary = async () => {
    ...
    try {
      let res = await fetch(`/api/news/adult/${id}/summary`);

      if (!res.ok) {
        res = await fetch(`/api/news/kids/${id}/summary`);
      }
